...
void sphereWallLubrication(const CollisionContext& ctx, dfloat gap)
{
    ...
    dfloat3 ut =
        G - un * n
        + R * cross_product(w_p, n);

    const dfloat mu = (TAU - 0.5) / 3.0;

    const dfloat C = 6.0 * M_PI * mu * R * R;

    dfloat3 f = -(C / h) * (un * n + ut);
...
void sphereSphereLubrication(const CollisionContext& ctx, dfloat gap)
{
    ...
    dfloat3 ut =
        G - un * n
        + R * cross_product(pc_i->getW(), n)
        + pc_j->getRadius() * cross_product(pc_j->getW(), n);
...
void sphereSphereAttraction(const CollisionContext& ctx, dfloat gap)
{
    ...
    const dfloat A_H = HAMAKER_CONST; // already in LBM units

    dfloat force_mag = (A_H * R) / (12.0 * h * h);
    dfloat3 f = -force_mag * n;
